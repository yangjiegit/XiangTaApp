...
public class SwipeFlingAdapterView extends BaseFlingAdapterView {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (adapterCount == 0) {
            ...
            if (mActiveCard != null && topCard != null && topCard == mActiveCard) {
                ...
                setTopView();
                if (viewBin != null) {
                    mActiveCard.setTranslationX(viewBin.x);
                    mActiveCard.setTranslationY(viewBin.y);
                    post(new Runnable() {
                        @Override
                        public void run() {
                            flingCardListener.resetCurrentView(new AnimatorListenerAdapter() {
                                @Override
    ...
    private void adjustChildView(View child, int index) {
        ...
        child.setTranslationY((int) (dpToPx((int) ITEM_SMALL_HIGH) * (n - p)));
    ...
    private void makeAndAddView(int index, View child) {
        ...
        /*if(index==0&&viewBin!=null){
            child.setTranslationX(viewBin.x);
            child.setTranslationY(viewBin.y);
        }else {
            ...
            if (mActiveCard != null) {
                ...
                flingCardListener = new FlingCardListener(mActiveCard, mAdapter.getItem(0),
                    ...
                    public void onCardExited() {
                        mUndoManager.add(mActiveCard, mAdapter.getItem(0));
                        mActiveCard = null;
                        p = 0f;
                    ...
                    public void onScroll(float scrollProgressPercent, String direction) {
                        mFlingListener.onScroll(scrollProgressPercent, direction);
...
//                        requestLayout();
                        ...
                        while (startingIndex < childCount) {
                            View newUnderChild = getChildAt(childCount - 1 - startingIndex);
                            if (newUnderChild != null && newUnderChild.getVisibility() != GONE) {
                                adjustChildView(newUnderChild, startingIndex);
                            }
                            startingIndex++;
                        }
    ...
    UndoManager.UndoBean viewBin = null;

    public void undo() {
        viewBin = mUndoManager.undo();
        if (viewBin != null) {
            mActiveCard = null;
            requestLayout();
        }
    ...
    public void setUndoListener(UndoManager.UndoLinstener undoListener) {
        mUndoManager.setUndoLinstener(undoListener);
    }
