...
import com.muse.xiangta.modle.NoticeIndexBean;
 ...
 * 动态 推荐
 */
public class DynamicNotificationFragment extends BaseListFragment<DynamicListModel> implements BaseQuickAdapter.OnItemClickListener, BaseQuickAdapter.OnItemChildClickListener {
    ...
    protected void requestGetData(boolean isCache) {
        ...
        Api.NoticeIndex(uId, uToken, String.valueOf(page), new StringCallback() {

            @Override
            public void onSuccess(String s, Call call, Response response) {
                Log.e("DynamicList", s);

                NoticeIndexBean data = new Gson().fromJson(s, NoticeIndexBean.class);

                if (StringUtils.toInt(data.getCode()) == 1) {
                    onLoadDataResult(data.getData().getList());
                } else {
