...
public class PushUtil implements Observer {
    ...
    private void PushNotify(TIMMessage msg){
        ...
        NotificationManager mNotificationManager = (NotificationManager) CuckooApplication.getInstances().getSystemService(CuckooApplication.getContext().NOTIFICATION_SERVICE);
        NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(CuckooApplication.getInstances());
        Intent notificationIntent = new Intent(CuckooApplication.getInstances(), HomeActivity.class);
        notificationIntent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP
                | Intent.FLAG_ACTIVITY_SINGLE_TOP);
        PendingIntent intent = PendingIntent.getActivity(CuckooApplication.getInstances(), 0,
...
//                .setNumber(++pushNum) //设置通知集合的数量
    ...
    public void reset(){
        NotificationManager notificationManager = (NotificationManager) CuckooApplication.getInstances().getSystemService(CuckooApplication.getInstances().NOTIFICATION_SERVICE);
